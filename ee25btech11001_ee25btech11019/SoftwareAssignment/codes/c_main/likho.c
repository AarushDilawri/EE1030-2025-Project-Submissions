...
void likh(const char* basename, double** A, int w, int h) {
    ...
    for (int i = 0; i < h; i++)
        for (int j = 0; j < w; j++) {
            if (A[i][j] < pixelmin) pixelmin = A[i][j];
            if (A[i][j] > pixelmax) pixelmax = A[i][j];
        }

    double range = (pixelmax - pixelmin);
    if (range < 1e-12) range = 1.0;

    // normalize to 0,255
    for (int i = 0; i < h; i++)
        for (int j = 0; j < w; j++) {
            double v = (A[i][j] - pixelmin) / range * 255.0;
...
void likh_rgb(const char* basename, double*** A_rgb, int w, int h) {
    ...
    for (int i = 0; i < h; i++) {
        for (int j = 0; j < w; j++) {
            for (int c = 0; c < 3; c++) {
                double v = (A_rgb[c][i][j] - pixelmin) / (pixelmax - pixelmin) * 255.0;
                if (v < 0) v = 0;
                if (v > 255) v = 255;
